...
<template>
    <div class="flex min-h-screen flex-col bg-background">
        ...
        <header class="sticky top-0 z-10 flex h-14 items-center justify-between border-b border-border bg-card px-4 shadow-sm pt-[env(safe-area-inset-top,0px)]" style="height: calc(3.5rem + env(safe-area-inset-top, 0px))">
            ...
            <div class="flex items-center gap-3 text-sm text-muted-foreground">
                ...
                <button
                    ...
                    :aria-label="permissionDenied ? 'Location permission denied' : (locationEnabled ? 'Sharing location — tap to stop' : 'Share location')"
        ...
        <main id="main-content" class="flex-1 pb-[calc(5rem+env(safe-area-inset-bottom,0px))]">
            <slot />
        </main>

        <!-- Bottom navigation — extends into iOS home bar safe area -->
        <nav aria-label="Main navigation" class="fixed bottom-0 left-0 right-0 z-10 flex border-t border-border bg-card pb-[env(safe-area-inset-bottom,0px)]">
            <Link
