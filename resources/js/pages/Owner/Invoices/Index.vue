...
<template>
    <OwnerLayout title="Invoices">
        ...
        <div class="mb-6 flex items-center justify-between">
            <h2 class="text-xl font-semibold text-foreground">Invoices</h2>
            <Link
        ...
        <section class="mb-6 grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-5">
            <div class="rounded-xl bg-card p-4 shadow ring-1 ring-border">
                <p class="text-xs font-medium uppercase tracking-wide text-muted-foreground">Outstanding</p>
                <p class="mt-2 text-2xl font-semibold text-blue-600 dark:text-blue-400">{{ formatCurrency(stats.outstanding_balance) }}</p>
                <p class="mt-0.5 text-xs text-muted-foreground">{{ stats.open_count }} open invoice{{ stats.open_count !== 1 ? 's' : '' }}</p>
            </div>
            <div class="rounded-xl bg-card p-4 shadow ring-1 ring-border">
                <p class="text-xs font-medium uppercase tracking-wide text-muted-foreground">Overdue</p>
                <p class="mt-2 text-2xl font-semibold" :class="Number(stats.overdue_balance) > 0 ? 'text-red-600 dark:text-red-400' : 'text-muted-foreground'">{{ formatCurrency(stats.overdue_balance) }}</p>
                <p class="mt-0.5 text-xs text-muted-foreground">{{ stats.overdue_count }} overdue invoice{{ stats.overdue_count !== 1 ? 's' : '' }}</p>
            </div>
            <div class="rounded-xl bg-card p-4 shadow ring-1 ring-border">
                <p class="text-xs font-medium uppercase tracking-wide text-muted-foreground">Paid This Month</p>
                <p class="mt-2 text-2xl font-semibold text-green-600 dark:text-green-400">{{ formatCurrency(stats.paid_this_month) }}</p>
            </div>
            <div class="rounded-xl bg-card p-4 shadow ring-1 ring-border">
                <p class="text-xs font-medium uppercase tracking-wide text-muted-foreground">Total Invoiced</p>
                <p class="mt-2 text-2xl font-semibold text-foreground">{{ formatCurrency(stats.total_invoiced) }}</p>
            </div>
            <div class="rounded-xl bg-card p-4 shadow ring-1 ring-border">
                <p class="text-xs font-medium uppercase tracking-wide text-muted-foreground">All Invoices</p>
                <p class="mt-2 text-2xl font-semibold text-foreground">{{ stats.total_count }}</p>
                <p class="mt-0.5 text-xs text-muted-foreground">lifetime</p>
            </div>
        </section>
        ...
        <div class="mb-4 flex flex-wrap items-center gap-2">
            <button
                ...
                :class="status === key ? 'bg-slate-800 text-white' : 'bg-card text-muted-foreground ring-1 ring-border hover:bg-accent'"
                @click="setStatus(key)"
            >
                {{ label }}
            </button>
            <input
                ...
                class="ml-auto w-56 rounded-lg border border-border bg-background px-3 py-1.5 text-sm text-foreground shadow-sm focus:outline-none focus:ring-2 focus:ring-slate-400"
            />
        </div>
            ...
            <table class="min-w-full divide-y divide-border text-sm">
                ...
                <tbody class="divide-y divide-border/50">
                    ...
                    >
                        ...
                        <td class="px-4 py-3 text-muted-foreground" @click.stop>
                        ...
                        <td class="px-4 py-3">
                            <span
                                ...
                                :class="STATUS_CLASSES[inv.status] ?? 'bg-muted text-muted-foreground'"
                            >
                                {{ statuses[inv.status] ?? inv.status }}
                            </span>
                        </td>
                        <td class="px-4 py-3 text-right text-foreground">{{ formatCurrency(inv.total) }}</td>
                        <td class="px-4 py-3 text-right font-medium" :class="Number(inv.balance_due) > 0 ? 'text-red-600 dark:text-red-400' : 'text-green-600 dark:text-green-400'">
                    ...
                    <tr v-if="invoices.data.length === 0">
                        <td colspan="8" class="px-4 py-16 text-center">
                            ...
                            <p class="mt-3 text-sm font-semibold text-foreground">No invoices yet</p>
                            <p class="mt-1 text-sm text-muted-foreground">Create your first invoice to get started.</p>
        ...
        <div v-if="invoices.total > 25" class="mt-4 flex flex-wrap gap-1">
            <template v-for="link in invoices.links" :key="link.label">
                <Link
                    ...
                    :class="link.active ? 'bg-slate-800 text-white' : 'border border-border text-muted-foreground hover:bg-accent'"
                    v-html="link.label"
                />
                <span
                    v-else
