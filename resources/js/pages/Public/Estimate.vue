    ...
    <div class="min-h-screen bg-background">
        ...
        <header class="bg-card shadow-sm">
            <div class="mx-auto max-w-4xl px-4 py-5 sm:px-6">
                <p v-if="estimate.organization" class="text-sm font-semibold text-muted-foreground">
                    {{ estimate.organization.name }}
                </p>
                <h1 class="text-2xl font-bold text-foreground">{{ estimate.title }}</h1>
                ...
                <p v-if="estimate.expires_at" class="mt-1 text-xs text-muted-foreground">
        ...
        <main class="mx-auto max-w-4xl px-4 py-8 sm:px-6">
            ...
            <!-- Accepted banner -->
            <div v-if="isAccepted" class="mb-6 rounded-xl bg-green-50 p-4 ring-1 ring-green-200 dark:bg-green-900/20 dark:ring-green-800">
                <p class="font-semibold text-green-700 dark:text-green-400">You've accepted this estimate.</p>
                <p class="mt-0.5 text-sm text-green-600 dark:text-green-400">
            ...
            <!-- Declined banner -->
            <div v-if="isDeclined" class="mb-6 rounded-xl bg-muted p-4 ring-1 ring-border">
                <p class="font-medium text-muted-foreground">You've declined this estimate.</p>
                <p class="mt-0.5 text-sm text-muted-foreground">Please contact us if you'd like to discuss further.</p>
            </div>

            <!-- Intro -->
            <p v-if="estimate.intro" class="mb-6 whitespace-pre-wrap text-muted-foreground">{{ estimate.intro }}</p>
            ...
            <div class="grid gap-4" :class="estimate.packages.length >= 3 ? 'md:grid-cols-3' : estimate.packages.length === 2 ? 'md:grid-cols-2' : 'max-w-sm'">
                <div
                    ...
                    :key="pkg.id"
                    class="relative flex cursor-pointer flex-col rounded-2xl bg-card shadow-sm ring-2 transition-all"
                    :class="[
                        pkg.is_recommended && !selectedTier ? 'ring-blue-500 shadow-blue-100' : '',
                        selectedTier === pkg.tier ? 'ring-blue-600 shadow-lg shadow-blue-100' : (pkg.is_recommended ? '' : 'ring-border'),
                    ]"
                ...
                >
                    ...
                    <!-- Description -->
                    <p v-if="pkg.description" class="border-b border-border px-5 py-3 text-sm text-muted-foreground">{{ pkg.description }}</p>

                    <!-- Line items -->
                    <ul class="flex-1 divide-y divide-border/50 px-5 py-3">
                        <li
                        ...
                        >
                            <div>
                                <p class="font-medium text-foreground">{{ li.name }}</p>
                                <p v-if="li.description" class="text-xs text-muted-foreground">{{ li.description }}</p>
                            </div>
                            <span class="shrink-0 font-medium text-foreground">{{ formatCurrency(li.total) }}</span>
                        </li>
                    </ul>

                    <!-- Select radio -->
                    <div v-if="isEditable" class="border-t border-border px-5 py-4 text-center">
                        <div
                            class="mx-auto flex h-6 w-6 items-center justify-center rounded-full border-2 transition"
                            :class="selectedTier === pkg.tier ? 'border-blue-600 bg-blue-600' : 'border-input'"
                        >
            ...
            <!-- Footer text -->
            <p v-if="estimate.footer" class="mt-6 whitespace-pre-wrap text-sm text-muted-foreground">{{ estimate.footer }}</p>
            ...
            <div v-if="isEditable" class="mt-8 flex flex-wrap justify-center gap-3">
                ...
                <button
                    type="button"
                    class="rounded-xl border border-border px-8 py-3 text-base font-medium text-muted-foreground hover:bg-accent disabled:opacity-50"
                    :disabled="submitting"
                    @click="decline"
                >
                    Decline
